        ...
        void Render(){
            ...
            if (j1){
                for (int i=0;i<j1->GetButtonCount();i++){
                    ...
                    RenderHelp::DrawSquareColor(Rect(32 * i,32,28,28),255,color,color2,100);
                }
                ...
                for (int i=0;i<j1->GetHatsCount();i++){
                    for (int b=0;b<j1->GetHatButtonCount();b++){
                        ...
                        RenderHelp::DrawSquareColor(Rect(32 * b,64 + 32*i,28,28),255,color,color2,100);
                    }
                }
                ...
                for (int i=0;i<j1->GetAxisCount();i++){
                    ...
                    if (s > 0){
                        RenderHelp::DrawSquareColor(Rect(400,96 + 32*i,s,28),0,255,0,preepr ? 255 : 100);
                    }else{
                        s = - s;
                        RenderHelp::DrawSquareColor(Rect(400 - s ,96 + 32*i,s,28),255,0,0,preepr ? 255 : 100);
                    }
                    RenderHelp::DrawSquareColor(Rect(400 - fSize,96 + 32*i,fSize * 2,28),0,255,255,255,true);
                    InputState sE = j1->GetAxisInputState(i*2 +1);
                    RenderHelp::DrawSquareColor(Rect(400 -fSize- 28 ,96 + 32*i,28,28),255,255,255,255,!(sE == PRESSED || sE == JUST_PRESSED));
                    sE = j1->GetAxisInputState(i*2 );
                    RenderHelp::DrawSquareColor(Rect(400 +fSize ,96 + 32*i,28,28),255,255,255,255,!(sE == PRESSED || sE == JUST_PRESSED));
